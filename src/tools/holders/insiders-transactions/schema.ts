...
export const holdersInsidersTransactionsInputSchema = z.object({
	symbol: z.string().describe('Asset identifier (ticker symbol). Must be a stock symbol.'),
	limit: z.number().min(1).max(5000).default(10).optional().describe('Maximum number of transaction records to return.'),
	...
	sort_by: z.array(
		z.object({
			selector: z.string().describe('Metric used for sorting (e.g., shares)'),
...
export type HoldersInsidersTransactionsInput = z.infer<typeof holdersInsidersTransactionsInputSchema>;
