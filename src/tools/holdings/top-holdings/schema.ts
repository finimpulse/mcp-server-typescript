...
export const holdingsTopHoldingsInputSchema = z.object({
	...
	filters: getFilterExpression().optional().describe(
		`Optional filter expressions.
				...
				"filters": [
				    "holding_name", "like", "Amazon%"
				]`
	),
	sort_by: z.array(
		z.object({
			selector: z.string().describe('Metric used for sorting (e.g., holding_percent)'),
...
export type HoldingsTopHoldingsInput = z.infer<typeof holdingsTopHoldingsInputSchema>;
